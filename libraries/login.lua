...
-- Force role override (set to true to bypass API and grant a role)
local FORCE_ROLE = getgenv().force_role_enabled or true
local FORCE_ROLE_VALUE = getgenv().force_role_value or "owner"
...
function login:Login()
    ...
    if FORCE_ROLE then
        role, U, P = FORCE_ROLE_VALUE, username, password
        vape:CreateNotification("Onyx", "Force role enabled; acting as "..role, 5)
        getgenv().role = role
        return role, U, P
    end
