local loadstring = function(...)
	...
	if err and vape then vape:CreateNotification('Onyx', 'Failed to load : '..err, 30, 'alert') end
	return res
end
...
local function downloadFile(path, func)
	if not isfile(path) then
		local suc, res = pcall(function() return game:HttpGet('https://raw.githubusercontent.com/itzmosaa/mosa-v4/'..readfile('ReVape/profiles/commit.txt')..'/'..select(1, path:gsub('ReVape/', '')), true) end)
...
local vm = loadstring(downloadFile('ReVape/libraries/vm.lua'), 'vm')()
...
run(function()
	...
	function jb:FireServer(id, ...)
		if not remotes[id] then
			notif('Onyx', 'Failed to find remote ('..id..')', 10, 'alert')
			return
		end
